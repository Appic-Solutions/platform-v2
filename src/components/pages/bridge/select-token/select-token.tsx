...
export default function BridgeSelectTokenPage({ stepHandler, setSelectedType, fromToken, toToken, swapTokensHandler, handleOptionSelect, selectedOption, amount, setAmount }: SelectTokenProps) {
  ...
  const disabled = () => {
    ...
    const isSourceWalletConnected = fromToken.chainType === "EVM" ? isEvmConnected : icpAgent;

    const isDestWalletValid = toWalletAddress && !toWalletValidationError;
    const isDestWalletConnected = toToken.chainType === "EVM" ? isEvmConnected : icpAgent;
  ...
  const getButtonText = () => {
    if (fromToken && toToken && fromToken.contractAddress === toToken.contractAddress && fromToken.chainId === toToken.chainId) {
    ...
    const isSourceWalletDisconnected = (fromToken?.chainType === "EVM" && !isEvmConnected) || (fromToken?.chainType === "ICP" && !icpAgent);
    ...
    const isDestWalletDisconnected = (toToken?.chainType === "EVM" && !isEvmConnected) || (toToken?.chainType === "ICP" && !icpAgent);
    return isDestWalletDisconnected ? "Connect Destination Wallet" : "Confirm";
  };

  return (
    <Box className={cn("flex flex-col gap-4 h-full md:min-h-[10vh]", "md:px-[65px] md:py-[55px] md:max-w-[617px]", "overflow-x-hidden", "transition-[max-height] duration-300 ease-in-out", amount && "lg:max-w-[1060px]", showWalletAddress ? "md:max-h-[770px]" : "md:max-h-[607px]")}>
      ...
      <div className="flex flex-col gap-x-4 flex-1 justify-between lg:flex-row lg:overflow-hidden w-full">
        ...
        <div className="flex flex-col justify-between h-full items-center gap-y-4 w-full lg:max-w-[482px] md:overflow-y-hidden">
          <div className="flex flex-col gap-y-4 w-full h-full">
            {/* TOKENS */}
            <div className={cn("relative flex w-full", fromToken && toToken ? "flex-col gap-y-4 sm:flex-row sm:gap-x-4" : "flex-col gap-y-4")}>
              <TokenCard
                token={fromToken}
                customOnClick={() => {
                ...
                label="From"
                className={cn(fromToken && "py-8 md:py-8 md:rounded-3xl", fromToken && toToken ? "max-h-min md:max-h-min" : "")}
              />
              <div
                className={cn(
              ...
              <TokenCard
                ...
                label="To"
                className={cn(toToken && "py-8 md:py-8 md:rounded-3xl", fromToken && toToken ? "max-h-min md:max-h-min" : "")}
              />
            </div>
            {/* AMOUNT INPUT */}
            {fromToken && toToken && (
              <AmountInput
                ...
                isWalletConnected={(() => {
                  if (fromToken.chainType === "EVM" && isEvmConnected) return true;
            ...
            <WalletAddressInput token={toToken} address={toWalletAddress} setAddress={setToWalletAddress} validationError={toWalletValidationError} onValidationError={setToWalletValidationError} show={showWalletAddress} avatar={getChainLogo(toToken?.chainId)} />
          </div>
          ...
          <div className={cn("flex items-center gap-x-2 w-full", "max-lg:hidden")}>
            ...
            <div onClick={() => setShowWalletAddress(!showWalletAddress)} className={cn("rounded-full bg-primary-buttons bg-opacity-20 flex items-center justify-center px-4 h-full", "transition-colors duration-300 cursor-pointer")}>
        ...
        {/* BRIDGE OPTIONS */}
        {amount && <BridgeOptionsList selectedOption={selectedOption} handleOptionSelect={handleOptionSelect} />}
      </div>
