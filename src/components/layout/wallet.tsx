...
const WalletPage = () => {
  ...
  return (
    <div className={cn("flex items-center justify-evenly gap-2 relative min-w-fit lg:h-[42px]", "rounded-full bg-[#faf7fd]/50 border border-[#ECE6F5]", "md:col-span-2 md:justify-self-end", (icpIdentity || isEvmConnected) && "px-3", "*:rounded-full")}>
      {!icpIdentity && !isEvmConnected ? (
        <>
            ...
            <Drawer>
              ...
              <DrawerContent>
                ...
                <div className="flex flex-col gap-4">
                  {!icpIdentity && <WalletCard connectWallet={() => connectIcp()} walletLogo="/images/logo/wallet_logos/icp.svg" walletTitle="Connect ICP Wallet" />}
                  {!isEvmConnected && <WalletCard connectWallet={() => openEvmModal()} walletLogo={getChainLogo(chainId)} walletTitle="Connect EVM Wallet" />}
                </div>
              </DrawerContent>
            </Drawer>
          </div>
          ...
          <div className="hidden md:block">
            <Popover>
              <PopoverTrigger className="w-full font-medium text-sm text-white py-2 px-3">Connect Wallet</PopoverTrigger>
      ...
      {icpBalance && <WalletPop logo="/images/logo/wallet_logos/icp.svg" title="Your ICP Wallet" balance={icpBalance} disconnect={handleDisconnectIcp} isLoading={icpLoading} address={icpIdentity?.getPrincipal().toString() || ""} />}

      {evmBalance && <WalletPop logo={getChainLogo(chainId)} title="Your EVM Wallet" balance={evmBalance} disconnect={handleDisconnectEvm} isLoading={evmLoading} address={evmAddress || ""} />}
    </div>
  );
};
