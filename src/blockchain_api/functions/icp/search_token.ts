...
export async function createIcpTokenFromCanister(
  canisterId: string,
  unAuthenticated_agent: HttpAgent,
): Promise<Response<IcpToken | null>> {
  try {
    ...
    // Fetch metadata
    const metadata: Array<[string, MetadataValue]> = (await tokenActor.icrc1_metadata()) as Array<
