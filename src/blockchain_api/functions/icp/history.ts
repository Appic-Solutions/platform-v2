...
export type DexAction =
	| { type: 'Swap'; data: { tokenIn: string; finalAmountIn: Erc20TokenAmount; finalAmountOut: Erc20TokenAmount; timestamp: string; tokenOut: string; swapType: SwapType, principal: string } }
	| { type: 'CreatedPool'; data: { token0: string; token1: string; timestamp: string; poolFee: number } }
	| { type: 'BurntPosition'; data: { amount0Received: Erc20TokenAmount; burntPosition: PositionKey; liquidity: Erc20TokenAmount; timestamp: string; amount1Received: Erc20TokenAmount } }
	| { type: 'IncreasedLiquidity'; data: { amount0Paid: Erc20TokenAmount; liquidityDelta: Erc20TokenAmount; amount1Paid: Erc20TokenAmount; timestamp: string; modifiedPosition: PositionKey } }
	| { type: 'CollectedFees'; data: { amount1Collected: Erc20TokenAmount; timestamp: string; position: PositionKey; amount0Collected: Erc20TokenAmount } }
	| { type: 'DecreasedLiquidity'; data: { amount0Received: Erc20TokenAmount; liquidityDelta: Erc20TokenAmount; timestamp: string; amount1Received: Erc20TokenAmount; modifiedPosition: PositionKey } }
	| { type: 'MintedPosition'; data: { amount0Paid: Erc20TokenAmount; liquidity: Erc20TokenAmount; createdPosition: PositionKey; amount1Paid: Erc20TokenAmount; timestamp: string } }
	| { type: 'CrosschainSwap'; data: { swapOrder: CrosschainSwapOrder; isRefunded: boolean; icpAmountOut: Opt<string> } };
...
export const get_transaction_history = async (
	...
	try {
		...
		console.log("transformed txs:", await transform_txs(txs, bridge_tokens, all_icp_tokens, available_evm_tokens));
		return {
...
const transform_txs = async (
	...
	const bridge_txs = txs.filter(
		(tx) => tx.type === 'EvmToIcp' || tx.type === 'IcpToEvm',
	);
	...
	for (const tx of crosschain_swaps) {
		...
		const status: Status = status_str === 'Successful' ? 'Successful' : status_str === 'Refunded' ? 'Refunded' : status_str === 'Quarantined' ? 'Failed' : 'Pending';
		...
		if (is_refunded && swap.refundToken) {
			...
			if (refund_chain === 'icp' || refund_chain === 'ICP') { // Assume ICP chain id
				refund_token = find_icp_token(all_icp_tokens, Principal.fromText(swap.refundToken));
			} else {
				refund_token = await find_or_fetch_evm_token(available_evm_tokens, refund_chain, swap.refundToken);
			}
		}
		const refund_amount = swap.refundAmount ?? undefined;
		const human_readable_refund_amount = refund_amount && refund_token ? new BigNumber(refund_amount)
...
const find_or_fetch_evm_token = async (tokens: EvmToken[], chainId: string, address: string): Promise<EvmToken> => {
	let token = tokens.find(
		(t) => t.chainId === Number(chainId) && t.contractAddress.toLowerCase() === address.toLowerCase(),
	);
	if (token) {
		return token;
	}
			...
			if (tx.type === 'EvmToIcp') {
				...
				const to_token = bridge_tokens.find(
					(token) =>
						...
						token.canisterId?.toLowerCase() ===
						transaction.icrcLedgerId?.toLowerCase(),
				)!;
...
export function parse_evm_to_icp_tx_status(status: EvmToIcpStatus): DepositTxStatus {
	switch (status) {
		case EvmToIcpStatus.PendingVerification: return "PendingVerification";
		case EvmToIcpStatus.Quarantined: return "Quarantined";
		case EvmToIcpStatus.Minted: return "Minted";
		case EvmToIcpStatus.Accepted: return "Accepted";
		case EvmToIcpStatus.Invalid: return "Invalid";
		default: throw new Error(`Unknown EVM to ICP transaction status: ${JSON.stringify(status)}`);
...
export function parse_icp_to_evm_tx_status(status: IcpToEvmStatus): WithdrawalTxStatus {
	...
	switch (status) {
		case IcpToEvmStatus.PendingVerification: return "PendingVerification";
		case IcpToEvmStatus.Accepted: return "Accepted";
		case IcpToEvmStatus.Created: return "Created";
		case IcpToEvmStatus.SignedTransaction: return "SignedTransaction";
		case IcpToEvmStatus.ReplacedTransaction: return "ReplacedTransaction";
		case IcpToEvmStatus.Reimbursed: return "Reimbursed";
		case IcpToEvmStatus.QuarantinedReimbursement: return "QuarantinedReimbursement";
		case IcpToEvmStatus.Successful: return "Successful";
		case IcpToEvmStatus.Failed: return "Failed";
		default: throw new Error(`Unknown ICP to EVM transaction status: ${JSON.stringify(status)}`);
	}
}
