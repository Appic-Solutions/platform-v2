...
import { IcpTokenType } from '@/blockchain_api/did/appic/appic_helper/appic_helper_types';
...
export const get_icp_tokens = async (
  unAuthenticatedAgent: HttpAgent,
): Promise<Response<IcpToken[]>> => {
  try {
    const response = await axios.get<{ data: ApiIcpToken[] }>(
      'https://api.appicdao.com/tokens/icp',
    );
