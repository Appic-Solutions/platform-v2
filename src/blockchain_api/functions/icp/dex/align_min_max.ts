...
export function alignMinOrMaxPrice({
	...
	if (min_price == 'min') {
		min_tick = is_token0_selected ? -887272 : 887272;
	...
	if (max_price == 'min') {
		max_tick = is_token0_selected ? -887272 : 887272;
	} else if (max_price == 'max') {
		max_tick = is_token0_selected ? 887272 : -887272;
	} else {
		// Calculate sqrt price from input
		const sqrtRatioX96 = new BigNumber(
	...
	console.log("TICK NOT ALIGNED", min_tick, max_tick);
	...
	console.log("Tick HELLOOOO", min_tickAligned, max_tickAligned);
	...
	return is_token0_selected
		? { min_price: min_priceInToken1.toString(), max_price: max_priceInToken1.toString(), min_tick: min_tickAligned, max_tick: max_tickAligned, is_token0_active, is_token1_active }
		: { min_price: new BigNumber(1).div(min_priceInToken1).toString(), max_price: new BigNumber(1).div(min_priceInToken1).toString(), max_tick: max_tickAligned, min_tick: min_tickAligned, is_token0_active, is_token1_active };
}
