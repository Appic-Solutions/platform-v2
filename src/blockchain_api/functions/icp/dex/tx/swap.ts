...
export async function approve_token_in(
  ...
  try {
    ...
    console.log(
      'allowance:',
      token_in_allowance.allowance.toString(),
      'approval amount:',
      quote.approvalAmount,
      sender_principal.toString(),
    );
    ...
    // token0
    if (
      BigNumber(token_in_allowance.allowance.toString()).isLessThanOrEqualTo(
    ...
    if ('Err' in swap_result) {
      ...
      return {
        ...
        result: {
          ...
          caption: `Swapped ${quote.amountIn} ${quote.tokenIn.symbol} to ${formatAmount(amount_out, quote.tokenOut.decimals)} ${quote.tokenOut.symbol}`,
          title: 'Swapped',
        } as SwapStatus,
        success: true,
      };
 ...
 */
export function createSwapArgs(quote: IcpQuote, recipient: Principal | undefined): SwapArgs {
