...
export function generate_decrease_liquidity_args({
  position,
  percentage,
}: RemoveLiquidityArgs): DecreaseLiquidityArgs | BurnPositionArgs {
  ...
  if (percentage == 100) {
    let amount0_min = BigNumber(position.token0_reserves_raw)
      .multipliedBy((100 - burn_slippage_percentage) / 100)
      .decimalPlaces(0)
      .toFixed();
    let amount1_min = BigNumber(position.token1_reserves_raw)
      .multipliedBy((100 - burn_slippage_percentage) / 100)
      .decimalPlaces(0)
      .toFixed();
    ...
    let liquidity = BigNumber(position.liquidity)
      .multipliedBy(percentage / 100)
      .decimalPlaces(0)
      .toFixed();
    let amount0_min = BigNumber(position.token0_reserves_raw)
      .multipliedBy(percentage / 100)
      .minus((100 - burn_slippage_percentage) / 100)
      .decimalPlaces(0)
      .toFixed();
    let amount1_min = BigNumber(position.token1_reserves_raw)
      .multipliedBy(percentage / 100)
