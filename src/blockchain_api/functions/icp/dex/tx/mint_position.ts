import { MintPositionArgs, Result_6 as MintPositionResult, CreatePoolArgs, Result_2 as CreatePoolResult } from "@/blockchain_api/did/appic/appic_dex/appic_dex_types";
import { idlFactory as appicDexIdlFactory } from "@/blockchain_api/did/appic/appic_dex/appic_dex.did";

import { GenerateMintPositionArgsParams, generate_mint_position_args } from "../calculate_mint_amounts";
...
export async function generate_args_and_approve_mint_position(args: GenerateMintPositionArgsParams,
	authenticated_agent: Agent, // HttpAgent , Agent
	unauthenticated_agent: HttpAgent,
): Promise<Response<MintPositionArgs | undefined>> {
	let { mint_position_args, token1_approval_amount, token0_approval_amount } = generate_mint_position_args(args);
	...
	try {
		...
		//
		// token0
		if (BigNumber(token0_allowance.allowance.toString()).isLessThan(BigNumber(token0_approval_amount))) {
			// In case of Native withdrawal
			const token0_approval_result = (await token0_actor.icrc2_approve({
				amount: BigInt(
					new BigNumber(token0_approval_amount)
						.toString(),
				),
				created_at_time: [],
				expected_allowance: [],
		...
		// token1
		if (BigNumber(token1_allowance.allowance.toString()).isLessThan(BigNumber(token1_approval_amount))) {
			// In case of Native withdrawal
			const token1_approval_result = (await token1_actor.icrc2_approve({
				amount: BigInt(
					new BigNumber(token1_approval_amount)
						.toString(),
				),
				created_at_time: [],
				expected_allowance: [],
