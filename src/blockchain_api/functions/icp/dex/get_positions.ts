...
interface GetSinglePositionArgs {
	key: CandidPositionKey;
	pools: Pool[];
	icpTokens: IcpToken[];
...
interface GetPositionsByOwnerArgs {
	owner: Principal;
	pools: Pool[];
	icpTokens: IcpToken[];
	unAuthenticatedAgent: Agent;
 ...
 */
export async function getSinglePosition(args: GetSinglePositionArgs): Promise<Response<Position | undefined>> {
	...
	try {
		...
		const positionsData = await dex_actor.get_positions_by_owner(key.owner) as Array<[CandidPositionKey, CandidPositionInfo]>;
 ...
 */
export async function getPositionsByOwner(args: GetPositionsByOwnerArgs): Promise<Response<Position[] | undefined>> {
	...
	try {
		...
		const positionsData = await dex_actor.get_positions_by_owner(owner) as Array<[CandidPositionKey, CandidPositionInfo]>;

		console.log(positionsData);
		return { result: createPositionObjects(pools, positionsData, icpTokens), message: "", success: true };
...
export function createPositionObjects(
	...
	return positions.map(([positionKey, positionInfo]) => {
		...
		const token0_usd_price = pool.token0_usd_price ? new BigNumber(pool.token0_usd_price) : new BigNumber(token0.usdPrice);
		const token1_usd_price = pool.token1_usd_price ? new BigNumber(pool.token1_usd_price) : new BigNumber(token1.usdPrice);
		...
		// Handle fees
		const feesToken0Owed = new BigNumber(positionInfo.fees_token0_owed).dividedBy(new BigNumber(10).pow(decimals0));
		const feesToken1Owed = new BigNumber(positionInfo.fees_token1_owed).dividedBy(new BigNumber(10).pow(decimals1));
