...
export function get_market_price(
  { token0, token1, is_token0_selected }: CalculatePriceArgs,
  ...
  // Calculate prices: price_token0_in_token1 = (USD price of token0) / (USD price of token1)
  ...
  const sqrtPriceX96 = P.sqrt().multipliedBy(new BigNumber(2).pow(96)).decimalPlaces(0).toString();
  ...
  let text = is_token0_selected
    ? `1 ${token0_symbol} = ${formatted_price_token0_in_token1} ${token1_symbol} (-)`
    : `1 ${token1_symbol} = ${formatted_price_token1_in_token0} ${token0_symbol} (-)`;

  let price = is_token0_selected ? price_token0_in_token1 : price_token1_in_token0;
...
export function calculate_price({
  ...
  is_token0_selected,
  price,
}: CalculatePriceArgs): MarketPrice {
  ...
  console.log(token0_decimals, token1_decimals);
  ...
  const text = is_token0_selected
    ? `1 ${token0_symbol} = ${formatted_price0_in_1} ${token1_symbol}`
    : `1 ${token1_symbol} = ${formatted_price1_in_0} ${token0_symbol}`;
