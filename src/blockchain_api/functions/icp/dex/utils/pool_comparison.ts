...
export function arePoolsEqual(pool1: CandidPoolId, pool2: CandidPoolId): boolean {
  ...
  const token0Equal = pool1.token0.toText
    ? pool1.token0.toText() === pool2.token0.toText()
    : pool1.token0 === pool2.token0;
  const token1Equal = pool1.token1.toText
    ? pool1.token1.toText() === pool2.token1.toText()
