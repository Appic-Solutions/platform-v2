...
const TWO_POW_128 = new BigNumber('2').pow(128);
const mulShift = (val: BigNumber, mulBy: string): BigNumber => {
	return new BigNumber(val)
		.multipliedBy(new BigNumber(mulBy))
		.dividedBy(TWO_POW_128)
...
export abstract class TickMath {
	...
	public static alignTickToPoolTickSpacing(poolFee: number, tick: string): string {
		let tick_spacing = getTickSpacing(poolFee)!;

		return BigNumber(tick).dividedToIntegerBy(tick_spacing).multipliedBy(tick_spacing).toFixed();
