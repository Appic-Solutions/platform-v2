...
args:ActiveTickArgs,
	...
	let min_tick = BigNumber(TickMath.MIN_TICK).dividedToIntegerBy(tick_spacing).multipliedBy(tick_spacing).toNumber();
	let max_tick = BigNumber(TickMath.MAX_TICK).dividedToIntegerBy(tick_spacing).multipliedBy(tick_spacing).toNumber();
...
export function transforom_active_ticks(
	args: ActiveTickArgs, ticks: CandidTickInfo[], min_tick: number, max_tick: number):
	ActiveTick[] {
	...
	for (let tick_index of sorted_tick_indices) {
		...
		let liquidity_gross_str = tick_map.has(tick_index) ? tick_map.get(tick_index)!.liquidity_gross.toString() : "0";
