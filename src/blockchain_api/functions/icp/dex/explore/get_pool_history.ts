...
export async function get_single_pool_data(
  ...
  try {
    ...
    if (historyResult.length === 0) {
      return {
        message: 'No history found for the specified pool',
        result: undefined,
        success: false,
      };
    }
    ...
    if (!pool) {
      return {
        message: 'Pool not found in provided pools list',
        result: undefined,
        success: false,
      };
    }
    ...
    return {
      message: `Failed to get single pool data: ${error}`,
      result: undefined,
      success: false,
    };
  }
...
function generateDexData(
  ...
  const calculateSwapVolume = (days: number): string => {
    ...
    poolHistories.forEach((history) => {
      ...
      const buckets =
        days <= 1
          ? history[1].hourly_frame
          : days <= 30
            ? history[1].daily_frame
            : history[1].yearly_frame;
      buckets.forEach((bucket) => {
        ...
        if (bucket.start_timestamp >= startTimestamp || bucket.end_timestamp > startTimestamp) {
          const volumeToken0 = new BigNumber(
            bucket.swap_volume_token0_during_bucket.toString(),
          ).dividedBy(new BigNumber(10).pow(token0.decimals));
          const volumeToken1 = new BigNumber(
            bucket.swap_volume_token1_during_bucket.toString(),
  ...
  const poolHistoriesData: PoolHistory[] = pools.map((pool) => {
    ...
    const processBuckets = (buckets: CandidHistoryBucket[]) => {
      ...
      buckets.forEach((bucket) => {
        ...
        // Fees in USD
        const feesToken0 = new BigNumber(
          bucket.fee_generated_token0_during_bucket.toString(),
        ).dividedBy(new BigNumber(10).pow(token0.decimals));
        const feesToken1 = new BigNumber(
          bucket.fee_generated_token1_during_bucket.toString(),
        ).dividedBy(new BigNumber(10).pow(token1.decimals));
        ...
        const price = sqrtPriceX96ToPrice(
          bucket.last_sqrtx96_price.toString(),
          token0.decimals,
          token1.decimals,
        ).toString();
        ...
        volumes_fees.push({
          volume: volumeUsd,
          fees: feesUsd,
          timestamp: bucket.start_timestamp.toString(),
        });
        prices.push({
          token0_in_token1: price,
          token1_in_token0: price_inverse,
          timestamp: bucket.start_timestamp.toString(),
        });
      });
    ...
    const last30DdBuckets = history[1].daily_frame.filter(
      (bucket) => bucket.start_timestamp >= last30dStart,
    );
    ...
    const apr = tvl_usd.isGreaterThan(0)
      ? annual_fees_usd.dividedBy(tvl_usd).multipliedBy(100).toString()
      : '0';
