...
function transformPool(
  poolId: CandidPoolId,
  poolState: CandidPoolState,
  getTokenInfo: (principal: Principal) => IcpToken | undefined,
): Pool {
  ...
  if (isToken0CkUSDC) {
    ...
    token1_usd_price = new BigNumber(10)
      .pow(decimals1)
      .div(P.multipliedBy(new BigNumber(10).pow(decimals0)))
      .toString();
  } else if (isToken1CkUSDC) {
    token1_usd_price = '1';
    // USD price of token0 = (P * 10^d0) / 10^d1
    token0_usd_price = P.multipliedBy(new BigNumber(10).pow(decimals0))
  ...
  pool.tvl_usd = reserves0
    .multipliedBy(token0UsdPriceBN)
    .plus(reserves1.multipliedBy(token1UsdPriceBN))
    .toString();

  const swapVolume0 = new BigNumber(pool.swap_volume0_all_time).dividedBy(
    new BigNumber(10).pow(decimals0),
  );
  const swapVolume1 = new BigNumber(pool.swap_volume1_all_time).dividedBy(
    new BigNumber(10).pow(decimals1),
  );
  pool.swap_volume_all_time_usd = swapVolume0
    .multipliedBy(token0UsdPriceBN)
    .plus(swapVolume1.multipliedBy(token1UsdPriceBN))
    .toString();

  const generatedSwapFee0 = new BigNumber(pool.generated_swap_fee0).dividedBy(
    new BigNumber(10).pow(decimals0),
  );
  const generatedSwapFee1 = new BigNumber(pool.generated_swap_fee1).dividedBy(
    new BigNumber(10).pow(decimals1),
  );
  ...
  pool.generated_swap_fee_all_time_usd = generatedSwapFee0
    .multipliedBy(token0UsdPriceBN)
    .plus(generatedSwapFee1.multipliedBy(token1UsdPriceBN))
    .toString();
...
export async function get_all_pools(
  ...
  try {
    ...
    let transformed_pools = pools.map(([poolId, poolState]) =>
      transformPool(poolId, poolState, getTokenInfo),
    );
