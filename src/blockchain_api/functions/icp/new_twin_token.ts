...
export const get_evm_token_and_generate_twin_token = async (
  ...
  try {
    if (base_chain.type == 'EVM') {
      ...
      const creation_fee =
        lsm_info.ls_creation_icp_fee.toString() == '0'
          ? BigNumber('0')
          : new BigNumber(lsm_info.ls_creation_icp_fee.toString()).plus(icp_transfer_fee * 2);
      ...
      if (typeof icp_topken == undefined || !icp_topken || typeof icp_topken == 'undefined') {
        return {
          result: undefined,
          message: 'No token available for provided canister id',
          success: false,
        };
      }
...
export const approve_icp_or_native_token = async (
  ...
  try {
    if (new_twin_metadata.base_chain.type == 'EVM') {
      ...
      if (new_twin_metadata.creation_fee == '0')
