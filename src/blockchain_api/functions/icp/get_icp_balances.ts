    ...
    const non_zero_balances = tokens_balances.filter((token) => {
      if (token.balance != "0") {
        totalBalanceUsd = totalBalanceUsd.plus(
          token.usdBalance as unknown as number
        );
        return true;
      }
    });
...
const get_single_token_balance = async (
  ...
  const idleFactory =
    tokenTypeLower === "dip20" || tokenTypeLower === "yc"
      ? dip20IdleFactory
      : icrcIdlFactory;
  ...
  try {
    ...
    console.error(
      `Error in get_single_token_balance for canisterId ${canisterId}:`,
      error
    );
