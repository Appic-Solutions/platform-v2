...
import {
  AddEvmToIcpTx,
  AddIcpToEvmTx,
  Result as NewEvmToIcpResult,
  Result_1 as NewIcpToEvmResult,
...
export const icrc2_approve = async (
  ...
  try {
    ...
    if (bridge_option.is_native) {
      ...
      let erc20_approve_fee = bridge_option.is_wrapped_icrc
        ? BigNumber(bridge_option.fees.approval_fee_in_erc20_tokens).dividedBy(2).toFixed()
        : bridge_option.fees.approval_fee_in_erc20_tokens;

      if (
        new BigNumber(erc20_allowance.allowance.toString()).isGreaterThanOrEqualTo(
          new BigNumber(bridge_option.amount).minus(erc20_approve_fee),
        )
      ) {
        return { result: 'Successful', success: true, message: '' };
      } else {
        const erc20_approval_result = (await erc20_actor.icrc2_approve({
          amount: BigInt(new BigNumber(bridge_option.amount).minus(erc20_approve_fee).toString()),
          created_at_time: [],
          expected_allowance: [],
          expires_at: [],
          fee: [],
...
export const request_withdraw = async (
  ...
  if (bridge_option.operator === 'Appic') {
    ...
    if (bridge_option.is_native) {
    ...
    } else if (bridge_option.is_wrapped_icrc == false) {
      ...
      try {
        ...
        const icrc_wrap_result = (await appic_minter_actor.wrap_icrc({
          ...
          recipient,
        } as WrapIcrcArg)) as WrapIcrcrResult;

        if ('Ok' in icrc_wrap_result) {
          return {
            result: icrc_wrap_result.Ok.native_block_index.toString(),
            message: '',
            success: true,
          };
        } else {
...
export const request_deposit = async (
  ...
  let amount = bridge_option.is_native
    ? BigNumber(bridge_option.amount)
        .minus(
          BigNumber(bridge_option.fees.max_fee_per_gas).plus(
            bridge_option.fees.approval_fee_in_native_token,
          ),
        )
        .toFixed()
  ...
  const encoded_deposit_function_data = encode_burn_or_deposit_function_data(
  ...
  try {
    ...
    const value = bridge_option.is_native ? BigInt(amount) : undefined;
