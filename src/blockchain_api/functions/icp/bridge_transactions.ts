...
export const icrc2_approve = async (
	...
	try {
		...
		if (bridge_option.is_native) {
			...
			let erc20_approve_fee = bridge_option.is_wrapped_icrc ? BigNumber(bridge_option.fees.approval_fee_in_erc20_tokens).dividedBy(2).toFixed() : bridge_option.fees.approval_fee_in_erc20_tokens;
...
export const request_deposit = async (
	...
	let amount = bridge_option.is_native ? BigNumber(bridge_option.amount).minus(BigNumber(bridge_option.fees.max_fee_per_gas).plus(bridge_option.fees.approval_fee_in_native_token)).toFixed() : bridge_option.amount;
