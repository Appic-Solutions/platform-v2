...
export async function get_evm_wallet_tokens_balances(
  ...
  const nonWrappedTokens = bridge_pairs.filter(
    (token): token is EvmToken =>
      token.chain_type === 'EVM' && !(token as EvmToken).is_wrapped_icrc,
  );
  ...
  try {
    ...
    const viemTokens = await Promise.all(
      wrappedTokens.map(async (token) => {
        ...
        try {
          ...
          console.error(
            `Error fetching balance for token ${contractAddress} on chain ${chainId}:`,
            error,
          );
          return {
            ...token,
    ...
    allTokens = [...ankrTokens, ...viemTokens].filter(
