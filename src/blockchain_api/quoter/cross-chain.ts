...
import {
  estimate_approval_fee,
  estimate_gas_fee,
  get_gas_price,
  NATIVE_TOKEN_ADDRESS,
} from '../functions/icp/get_bridge_options';
...
export async function fetchCrossChainQuote({
  ...
  console.log(feeIn);
  const amountInRawForApi = new BigNumber(amount)
    .multipliedBy(bn10.pow(decimalsIn))
  ...
  try {
    ...
    if (response.data.success === true) {
      ...
      if (chainA != 'icp') {
        ...
        let { max_fee_per_gas, max_priority_fee_per_gas } = await get_gas_price(
          viemChain,
          chainAConfig.rpc_url,
        );
        let gas_limit = BigNumber(data.steps[0].quote.gasLimit!).toFixed();
        let { approval_gas, total_approval_fee } = estimate_approval_fee(
          max_fee_per_gas,
          tokenA.toLowerCase() == NATIVE_TOKEN_ADDRESS.toLowerCase(),
        );
        ...
        let human_readable_native_token_fee = BigNumber(total_native_token_fee)
        ...
        nativeTokenFees = {
          ...
          totalNativeFeeUSD: BigNumber(nativeToken?.usdPrice!)
            .multipliedBy(human_readable_native_token_fee)
            .toFixed(2),
      ...
      let chainBConfig =
        chainB != 'icp' ? chains.find((chain) => chain.chainId == chainB)! : undefined;
