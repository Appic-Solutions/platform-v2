...
async function fetchICPQuote(
	...
	let amountIn = BigNumber(amount).multipliedBy(BigNumber(10).pow(tokenIn.decimals)).minus(tokenIn.fee!).toString();
	try {
		...
		response.data.data.amountOut = BigNumber(response.data.data.amountOut).minus(tokenOut.fee!).toString();
		response.data.data.minAmountOut = BigNumber(response.data.data.minAmountOut).minus(tokenOut.fee!).toString();
