export const idlFactory = ({ IDL }) => {
	...
	const MinterInfo = IDL.Record({
		...
		'last_native_token_usd_price_estimate': IDL.Opt(
			NativeTokenUsdPriceEstimate
		),
	...
	return IDL.Service({
		...
		'retrieve_deposit_status': IDL.Func(
			[IDL.Text],
			[IDL.Opt(DepositStatus)],
			['query'],
		),
		'retrieve_swap_status_by_hash': IDL.Func(
			[IDL.Text],
			[IDL.Opt(SwapStatus)],
			['query'],
		),
		'retrieve_swap_status_by_swap_tx_id': IDL.Func(
			[IDL.Text],
			[IDL.Opt(SwapStatus)],
			['query'],
		),
		'retrieve_withdrawal_status': IDL.Func(
			[IDL.Nat64],
			[RetrieveWithdrawalStatus],
			[],
		),
		'smart_contract_address': IDL.Func(
			[],
