      ...
      {!filteredData.length ? (
        ...
        filteredData.map((item, idx) => {
          ...
          return (
            ...
            >
              {/* Date & Time */}
              <div className="flex items-center justify-between gap-x-4 text-sm font-bold text-[#898989]">
                <p>{date}</p>
                <p>{time}</p>
              </div>
...
function Footer({ item, isSwapped, status }: { item: any; isSwapped: boolean; status: string }) {
  const { type } = item;
  return (
    <div className="flex items-center justify-between gap-x-4 text-xs font-bold text-[#898989] *:flex *:flex-1 *:flex-col *:justify-center">
      <div>
        <p>
          {isSwapped
