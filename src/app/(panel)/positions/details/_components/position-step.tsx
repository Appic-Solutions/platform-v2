...
import { CreatePositionStep, CreatePositionStepDetail } from '@/app/(panel)/positions/types';
    ...
    >
      ...
      {currentStep.status && (
        <div className="flex flex-col gap-y-2">
          ...
          <p className="text-sm font-semibold text-[#9F9F9F]">{currentStep.errorMessage}</p>
        </div>
      )}
      {currentStep.step === 2 && currentStep.status === 'successful' && (
        <>
          <p className="pb-2 text-sm font-semibold text-[#9F9F9F]">
