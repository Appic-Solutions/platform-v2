import { usePositionDetailsStore } from '@/app/(panel)/positions/_store/usePositionDetailsStore';
import SetUserWalletBalanceButton from '@/app/(panel)/positions/create/_components/SetUserWalletBalanceButton';
import { FormattedPosition } from '@/app/(panel)/positions/types';
...
const AddLiquidityInput = ({ position, userTokenBalance, isAmountZero }: Props) => {
  ...
  return (
    <div
      className={cn(
        'group rounded-[20px] bg-box-border-gradient p-0.5 text-white backdrop-blur-[30px]',
      )}
    >
