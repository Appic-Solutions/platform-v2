...
const InvestPeriod = ({
  ...
  return (
    <div className="flex items-center gap-x-12">
      <p className="text-nowrap text-[18px] text-black dark:text-white">Repeat Every</p>
      <div className="flex items-center gap-x-4">
        <span className="text-[18px] text-black dark:text-white">{investmentPeriod}</span>
        ...
        <Select defaultValue={selectedCycle} onValueChange={(value) => setSelectedCycle(value)}>
          <SelectTrigger
            className={cn(
              'w-min gap-x-4 rounded-lg border-[1px] border-white py-6 text-sm text-[#0A0A0B]',
              'bg-white/50 dark:bg-[#F5F5F5]',
            )}
          >
            <SelectValue />
