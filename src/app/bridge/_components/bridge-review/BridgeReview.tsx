  ...
  if (selectedOption) {
    return (
      <Box className={cn('justify-normal animate-slide-in opacity-0', 'md:max-w-[612px] md:h-[607px] md:px-9 md:py-8')}>
        <BoxHeader title="Bridge Review" onBack={() => setActiveStep(2)} />
          ...
          >
              ...
              <div className="flex items-center gap-x-3">
                ...
                <p
                  className={cn(
                    ...
                    selectedOption.human_readable_estimated_return.length > 7 && 'text-ellipsis w-36',
                  )}
                >
                  {selectedOption.human_readable_estimated_return}
                </p>
              </div>
              <div className="flex flex-col gap-y-3 items-end">
                <div className="px-4 py-1 rounded-2xl flex items-center gap-x-1 bg-white">
                  <span className={cn('text-xs lg:text-sm text-blue-600')}>via {selectedOption.via}</span>
            ...
            <div className="mt-4 w-full border-t border-gray-200 dark:border-gray-700 pt-4">
              <div className="space-y-4">
                <p className="text-sm font-medium">SelectedOption Details:</p>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted">Network Fee:</span>
                    <span>
                      {selectedOption.fees.human_readable_max_network_fee} {selectedOption.fees.native_fee_token_symbol}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
