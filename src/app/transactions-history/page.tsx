...
import { TRANSACTION_TAB_ITEMS, TransactionTabItem } from '@/common/constants/layout/transactions';
...
const TransactionsPage = () => {
  const [selectedTab, setSelectedTab] = useState<TransactionTabItem['value']>(TRANSACTION_TAB_ITEMS[0].value);
  const [selectedTransactions, setSelectedTransactions] = useState<Transaction[]>(sampleTransactions);

  useEffect(() => {
    if (window.location.hash) {
      const newTab = window.location.hash.slice(1) as TransactionTabItem['value'];
  ...
  useEffect(() => {
    if (selectedTab === 'bridge') {
      setSelectedTransactions(sampleTransactions.filter((transaction) => transaction.type === 'bridge'));
    } else if (selectedTab === 'auto-invest') {
      setSelectedTransactions(sampleTransactions.filter((transaction) => transaction.type === 'auto-invest'));
    } else if (selectedTab === 'advanced') {
      setSelectedTransactions(sampleTransactions.filter((transaction) => transaction.type === 'advanced'));
    } else {
  ...
  return (
    ...
    >
      ...
      >
        <div className={cn('w-full h-full relative m-auto', 'md:max-w-[617px] md:max-h-[750px]')}>
          {/* mobile title */}
          <div
            className={cn(
          ...
          >
            <BackButton />
            <p className="text-xl md:text-3xl font-bold">{tabTitleConverter(selectedTab)}</p>
          </div>
          <TransactionTabsList
            customOnClick={(tab) => {
          ...
          >
            ...
            <div className="w-full overflow-y-scroll">
              <TransactionTabsContent value={selectedTab} transactions={selectedTransactions} />
            </div>
          </Box>
        </div>
      </Tabs>
