...
export default function TokenListPage({ prevStepHandler, setTokenHandler, selectedToken }: TokenListProps) {
  ...
  const fetchBridgeTokens = async () => {
    try {
      if (unauthenticatedAgent) {
        ...
        const bridge_tokens = await get_bridge_pairs(unauthenticatedAgent, JSON.parse(icp_tokens || "[]"));

        console.log(bridge_tokens);
        setBridgeTokens(bridge_tokens);
      }
    } catch (error) {
  ...
  const filteredTokens = useMemo(() => {
    ...
    return bridgeTokens
      ...
      .filter((token) => token.name.toLowerCase().includes(searchQuery) || token.symbol.toLowerCase().includes(searchQuery) || token.contractAddress?.toLowerCase().includes(searchQuery) || token.canisterId?.toLowerCase().includes(searchQuery));
  ...
  return (
    <Box className={cn("justify-normal animate-slide-in opacity-0", "md:max-w-[612px] md:h-[607px] md:px-9 md:py-8")}>
      ...
      <input
        ...
        className={cn("border-[#1C68F8] dark:border-[#000000] rounded-md py-2 px-3 md:mt-7 mb-6", "bg-white/50 dark:bg-white/30 text-black dark:text-white", "placeholder:text-black/50 dark:placeholder:text-white/50", "w-full")}
      />
