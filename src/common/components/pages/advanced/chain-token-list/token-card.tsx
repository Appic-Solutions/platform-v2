...
const TokenCard = ({ token, onClick, isSelected }: { token: EvmToken | IcpToken; onClick: () => void; isSelected: boolean }) => (
  <div className={cn("flex items-center gap-x-5 cursor-pointer group duration-200 rounded-md p-2", "hover:bg-[#F5F5F5] dark:hover:bg-[#2A2A2A]", isSelected && "bg-[#F5F5F5] dark:bg-[#2A2A2A]")} onClick={onClick}>
      ...
      <div className="overflow-hidden h-5">
        <div className="flex flex-col transition-transform duration-300 group-hover:-translate-y-5">
          ...
          <p className="text-sm font-semibold text-[#6E6E6E] dark:text-[#B5B3B3] truncate flex items-center gap-x-2">
            ...
            <Link
              href={token.chainType === "EVM" ? `https://etherscan.io/token/${token.contractAddress}` : `https://dashboard.internetcomputer.org/canister/${token.canisterId}`}
              target="_blank"
