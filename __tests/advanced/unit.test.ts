...
describe('apiService', () => {
  ...
  describe('fetchTwinToken', () => {
    it('returns the twin token metadata on successful API call', async () => {
      ...
      expect(get_evm_token_and_generate_twin_token).toHaveBeenCalledWith(
        '1',
        '0x123',
        mockHttpAgent,
      );
    ...
    it('throws an error when chainId is invalid', async () => {
      ...
      await expect(apiService.fetchTwinToken('', '0x123', mockHttpAgent)).rejects.toThrow(
        'Invalid chainId',
      );
    });
    ...
    it('throws an error when contractAddress is invalid', async () => {
      ...
      await expect(apiService.fetchTwinToken('1', '', mockHttpAgent)).rejects.toThrow(
  ...
  describe('approveICP', () => {
    ...
    it('throws an error on API failure with specific message', async () => {
      ...
      await expect(apiService.approveICP(mockNewTwinMetadata, mockAgent)).rejects.toThrow(
        'Approval denied',
      );
    });
